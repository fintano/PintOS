...
/* Inserts ELEM following types */

/* TYPES  INFO < PRIORITY , WAKEUPTIME > */

void list_push_types(struct list *list, struct list_elem *elem, enum types type){

		struct thread *t = thread_current();
		//struct thread *t_list;
		struct list_elem *e;
		int is_end = 1;
		...
		//if(types == PRIORITY){
				if(!list_empty(list)){
					for( e = list_begin(list) ; e!= list_end(list) ; e = list_next(e)){
						...
						//printf(" NAME %s , WAKETIME %d \n", t_list->name,t_list->wakeUpTime);

						if(type == PRIORITY && t->priority > t_list->priority){
							list_insert(e,elem);
							is_end = 0;
							break;
						}

						/* 현재 쓰레드의 wakeuptime이 리스트안에있는 wakeuptime보다 작을때 */
						/* 그 위치에 넣어준다                                              */

						else if(type == WAKEUPTIME && t->wakeUpTime <= t_list->wakeUpTime){
						//	printf(" %s < %s \n " , t->name, t_list->name);
							list_insert(e,elem);
							is_end = 0;
							break;
				...
				if(is_end){ // insert at last
						//printf(" %s \n ", t->name);
						list_push_back(list,elem);
				}
		//}
		//else if(types == WAKEUPTIME){
