...
// 성철 : thread_chreate 부분에 새로 구현한 parent, child 부분 생성하기 위한 부분 추가 (2014_10_25)
// 추가로 init_thread 부분에 parent, child, exit_status 초기화
               ...
               thread_func *function, void *aux) 
{
  ...
  //printf("tid : %d , it must be same as childstatus and be printed earlier than wait func\n", tid);
  /* Add to run queue. */
  thread_unblock (t);
...
thread_exit (void) 
{
		...
		//printf(" ************ thread_exit lockUser value ? %d \n",lockUser);
		//releaseUser(&lockUser);
		//printf(" ************ thread_exit lockUser value ? %d \n",lockUser);
		//printf("********thread_exit \n");
  ASSERT (!intr_context ());
