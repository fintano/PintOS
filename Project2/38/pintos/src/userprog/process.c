		...
		for(e=list_begin(&(t->childList));e!=list_end(&(t->childList));e=list_next(e)){
			...
			if(child_tid == child->tid){
					if(child->status == THREAD_DYING){
						...
						//printf("PARENT PID : %d \n", t->tid);
						releaseUser(&t->lock2User);	
						acquireUser(&child->lockUser); 
						//printf("THREAD_RUNNING childStatus %d, PARENT PID %d \n", t-> exit_status,t->tid);
...
load (const char *file_name, void (**eip) (void), void **esp) 
{
  ...
  while( (token = strtok_r(ptr," \n",&tmp)) != NULL ){
		//strlcpy(arg_stack[arg_cnt],token,PGSIZE);
...
*eip = (void (*) (void)) ehdr.e_entry;
  ...
  //hex_dump((uint32_t)*esp,*esp, PHYS_BASE - (*esp), true);
  success = true;
