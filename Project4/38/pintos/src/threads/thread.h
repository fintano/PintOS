...
struct thread
  {
    ...
    /* Owned by userprog/process.c. */
		/*  1024개의 PDE 배열을 가지고 있다. 
			 보통 thread_current() -> pagedir 로 접근 
		   upage가 저장이 안되어있으므로 어떻게 mapping이 되어있는지 모름 */
	...
	//add project 1
	struct thread* parent;	//현재 thread의 부모를 가리키기 위한 포인터
	struct list childList;	//현재 thread의 자식을 가리키기 위한 포인터
	struct list_elem childElem;
	int exit_status;		//exit status를 저장

	int lockUser;
	int lock2User;

	int lockUser_missingfile;
...
//project 3
void thread_get_load_avg_update(void);
//void thread_get_recent_cpu_update(void);
